...
const App = () => {
    ...
    const congee = location.state.conge;
    const [info, setinfo] = useState({
        technician: congee.technician,
      Date: [dayjs(congee.startDate), dayjs(congee.endDate)],
        returnDate: dayjs(congee.returnDate),
        type: congee.type,
    });
    ...
    const onFinishGeneral = (values) => {
        ...
        axios.patch(`https://opti-track-1.onrender.com/conge/${congee.key}`, conge,{
            ...
            .then((response) => {
                message.success('congé updated successfully');
                navigate('/congés');
            }).catch((error) => {
            if (error.response.status === 410) {
                ...
                message.error('Failed to edit congé');
            }
        });
    }
